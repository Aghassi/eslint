 ...
 * Processes an individual file using ESLint. Files used here are known to
 * exist, so no need to check that here.
 * TODO: Add worker farm here by moving this to a file for the worker pool to call on
 * @param {string} filename The filename of the file being checked.
 * @param {Object} configHelper The configuration options for ESLint.
 * @param {Object} options The CLIEngine options object.
 * @param {Linter} linter Linter context
 * @returns {Promise} The results for linting on this file.
 * @private
 */
function processFileAsync(filename, configHelper, options, linter) {
    return new Promise((resolve, reject) => {
        fs.readFile(path.resolve(filename), "utf8", (text, err) => {
            if (err) {
                reject(err);
            }
            processText(
...
class CLIEngine {
    ...
    /**
     * Outputs fixes from the given results to files asynchronously
     * @param {Object} report The report object created by CLIEngine.
     * @returns {Promise} a Promise that will resolve after the file is written to disk.
     * Uses `fs.writeFile` under the hood
     */
    static outputFixesAsync(report) {
        report.results.filter(result => result.hasOwnProperty("output")).forEach(result => new Promise((resolve, reject) => {
            fs.writeFile(result.filePath, result.output, (res, err) => {
                if (err) {
                    reject(err);
                }
    ...
    /**
     * Executes the current configuration on an array of file and directory names.
     * This version will process files via a Promise mechanism under the hood, and will
     * finish via `Promise.all` once complete internally. This function is still bound by
     * the synchronous checks for cache.
     * @param {string[]} patterns An array of file and directory names.
     * @returns {Promise} The results for all files that were linted.
     */
    executeOnFilesAsync(patterns) {
        const options = this.options,
            lintResultCache = this._lintResultCache,
            configHelper = this.config;
        ...
        fileList.forEach(fileInfo => {
             ...
             * Should we do a `catch` here? If so what do we want?
             */
            promisedList.push(processFileAsync(fileInfo.filename, configHelper, options, this.linter).then(result => {

                // If there is a cache, and we have a message, store it
                if (options.cache && result.messages.length) {
        ...
        /**
         * Once all of the files are processed, we can calculate stats etc.
         */
        return Promise.all(promisedList).then(results => {
            const stats = calculateStatsPerRun(results);
    ...
    executeOnText(text, filename, warnIgnored) {
        ...
        if (resolvedFilename && ignoredPaths.contains(resolvedFilename)) {
            ...
            // TODO: Add a worker farm here to speed this up
            results.push(
                processText(
                    text,
                    configHelper,
