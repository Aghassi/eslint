...
class CLIEngine {
    ...
    static outputFixesAsync(report) {
        report.results.filter(result => result.hasOwnProperty("output")).forEach(result => fs.writeFile(result.filePath, result.output));
    }
    ...
    executeOnFilesAsync(patterns) {
        ...
        fileList.map(fileInfo => {
             ...
             */
            const filePromise = processFileAsync(fileInfo.filename, configHelper, options, this.linter).then(result => {
